...
- Read an issue and comments with `gh issue view <number> --repo pong1013/local-k3s --comments` and request structured JSON fields when the caller needs exact metadata.
- List issues with `gh issue list --repo pong1013/local-k3s --state <state> --json ...` and use an explicit filter rather than assuming every open issue belongs to the Feature Run.
...
- Create an issue with `gh issue create --repo pong1013/local-k3s --title <title> --body-file <file>`.
